...
public class WaveManager : MonoBehaviour {
	...
	void Spawn(Wave.Element element) {
        ...
        if (playerHealth.currentHealth <= 0f || spawningPoints.Length == 0)
        {
			return;
		}
        ...
        //reset l'iterateur si besoin
        if (spawningPointsIterator >= spawningPoints.Length || spawningPointsIterator >= waveNumber )
            spawningPointsIterator = 0;

        //trouve le premier point viable 
        while (!spawningPoints[spawningPointsIterator].isActiveAndEnabled)
        {
        ...
        GameObject enemy =  spawningPoints[spawningPointsIterator].Spawn(element.enemy);
        HealthManager healthMan = enemy.GetComponent<HealthManager>();
		// Augmente la santé de l'ennemi en fonction de la difficulté

        if(difficulty > 0)
            healthMan.RaiseInitHealth(healthMan.initHealth / difficulty -1 * difficulty);
